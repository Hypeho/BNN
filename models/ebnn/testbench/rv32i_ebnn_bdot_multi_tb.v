...
module rv32i_ebnn_bdot_multi_tb;
    ...
    reg [31:0] expected_activation [0:11];
    reg [31:0] expected_checksum [0:0];
    ...
    reg [1023:0] activation_file;
    reg [1023:0] checksum_file;
    integer sample_id;
    ...
    integer activation_match;
    integer prediction_match;
    integer metadata_match;
    ...
    task finish_and_check;
        begin
            ...
            for (i = 0; i < 12; i = i + 1)
                if (act0_mem.mem[i] !== expected_activation[i]) activation_match = 0;
            checksum_match = (dmem[4] === expected_checksum[0]);
            prediction_match = (dmem[1] == expected_prediction);
            metadata_match = (dmem[2] == ground_truth_label) &&
                             (dmem[3] == (dmem[1] == ground_truth_label)) &&
                             (dmem[6] == sample_id);
            overall_match = (dmem[0] == 1) && prediction_match && score_match &&
                            checksum_match && activation_match && metadata_match &&
                            (dmem[5] == 3250) && (bdot_count == 3250) &&
                            (block_read_count == 3270) && (bcfg_count == 2) &&
                            (error_count == 0);

            $display("EBNN_MULTI detail sample=%0d dataset_index=%0d label=%0d cycles=%0d bdot=%0d blocks=%0d bcfg=%0d errors=%0d",
                     sample_id, sample_id, ground_truth_label, cpu_cycles,
                     bdot_count, block_read_count, bcfg_count, error_count);
            $display("EBNN_MULTI status=%0d prediction=%0d expected=%0d prediction_match=%0d score_match=%0d checksum_match=%0d activation_match=%0d",
                     dmem[0], dmem[1], expected_prediction, prediction_match,
                     score_match, checksum_match, activation_match);
            $display("EBNN_MULTI activation_checksum=%08x software_bdot=%0d",
                     dmem[4], dmem[5]);
            $display("EBNN_MULTI score_bits=%08x,%08x,%08x,%08x,%08x,%08x,%08x,%08x,%08x,%08x",
                     dmem[16], dmem[17], dmem[18], dmem[19], dmem[20],
                     dmem[21], dmem[22], dmem[23], dmem[24], dmem[25]);
            if (overall_match) begin
                $display("RESULT model=EBNN sample_id=%0d dataset_index=%0d ground_truth_label=%0d expected_prediction=%0d actual_prediction=%0d prediction_match=PASS score_match=PASS checksum_match=PASS activation_match=PASS cycles=%0d bdot_count=%0d block_count=%0d status=%0d result=PASS",
                         sample_id, sample_id, ground_truth_label,
                         expected_prediction, dmem[1], cpu_cycles, bdot_count,
                         block_read_count, dmem[0]);
                $display("TB PASS: rv32i_ebnn_bdot_multi sample=%0d", sample_id);
            end else begin
                $display("RESULT model=EBNN sample_id=%0d dataset_index=%0d ground_truth_label=%0d expected_prediction=%0d actual_prediction=%0d prediction_match=%s score_match=%s checksum_match=%s activation_match=%s cycles=%0d bdot_count=%0d block_count=%0d status=%0d result=FAIL",
                         sample_id, sample_id, ground_truth_label,
                         expected_prediction, dmem[1],
                         prediction_match ? "PASS" : "FAIL",
                         score_match ? "PASS" : "FAIL",
                         checksum_match ? "PASS" : "FAIL",
                         activation_match ? "PASS" : "FAIL",
                         cpu_cycles, bdot_count, block_read_count, dmem[0]);
                $display("TB FAIL: eBNN Binary-MNIST Multiple-Input mismatch");
            end
            $finish;
        end
    endtask
    ...
    always @(posedge cpu_clk) begin
        if (reset) begin
            ...
            if (cpu_cycles >= TIMEOUT_CPU_CYCLES) begin
                finished = 1;
                $display("RESULT model=EBNN sample_id=%0d dataset_index=%0d ground_truth_label=%0d expected_prediction=%0d actual_prediction=%0d prediction_match=FAIL score_match=FAIL checksum_match=FAIL activation_match=FAIL cycles=%0d bdot_count=%0d block_count=%0d status=%0d result=FAIL",
                         sample_id, sample_id, ground_truth_label,
                         expected_prediction, dmem[1], cpu_cycles,
                         bdot_count, block_read_count, dmem[0]);
                $display("TB FAIL timeout pc=%08x", pc);
                $finish;
            end
        end
    end
    ...
    initial begin
        ...
        if (!$value$plusargs("ACTIVATION=%s", activation_file)) $fatal(1, "missing ACTIVATION");
        if (!$value$plusargs("CHECKSUM=%s", checksum_file)) $fatal(1, "missing CHECKSUM");
        ...
        $readmemh(activation_file, expected_activation);
        $readmemh(checksum_file, expected_checksum);
